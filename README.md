...
```bash
pnpm test      # 20 тестов: движок, контракт ответа модели, согласованность кривой
pnpm lint
pnpm build
...
Двадцать тестов. Среди них — эталонные значения прямо из технического задания:
...
Дешевле 62 единиц допустимого набора из пяти решений не существует вовсе.
Переход с 65 на 70 покупает 0.88 балла,
а следующие 30 единиц — всего 0.57.
Для управленца это прямой ответ на вопрос, стоит ли добивать бюджет до потолка: после
70 единиц деньги почти перестают конвертироваться в качество городской среды.
